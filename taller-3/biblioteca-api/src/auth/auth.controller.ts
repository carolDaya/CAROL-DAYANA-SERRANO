...
export class AuthController { // Declaración de la clase AuthController
  ...
  register(@Body() dto: RegisterDto) { // Declaración del método para registro
    if (!dto.email || !dto.password || !dto.name) // Validación de campos requeridos
  ...
  async login(@Body() dto: LoginDto) { // Declaración del método para login
